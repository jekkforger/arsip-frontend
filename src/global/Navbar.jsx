...
/** Ambil role dari URL prefix */
function getRoleFromPath(pathname) {
  if (pathname.startsWith("/scanner")) return "scanner";
  if (pathname.startsWith("/pegawai")) return "pegawai";
  if (pathname.startsWith("/kaban")) return "kaban";
...
const NAV_BY_ROLE = {
  ...
  pegawai: [
    ...
    },
    {
      key: "input",
      label: "Input Dokumen",
      icon: inputSvg, // kalau punya icon input sendiri, ganti di sini
      to: "/pegawai/input",
    },
    {
      key: "favorite",
...
export default function Navbar() {
  ...
  // fallback kalau role belum kebaca
  const navItems = NAV_BY_ROLE[role] || NAV_BY_ROLE.kaban;
    ...
    <aside className="hidden md:flex fixed left-0 top-0 z-50 h-screen w-[280px] flex-col bg-[#1D4ED8] text-white">
      ...
      <div className="px-8 pb-8 pt-6">
        <button
          ...
          onClick={() => {
            // taro logic logout lu di sini (clear token / localStorage)
            // contoh: localStorage.clear();
